...
- Instead of opening the file descriptor for the requested CPU, and sharing it for subsequent operations until caller decides to manually invoke `Close()`, we let the MSR interface handle
releasing the file descriptor once the subsequent operations finished. This is done by MSR interface taking closure as an argument, leaving its implementation to the users of this library. Additionally,
the "example" implementation is given by `ReadMSR` and `WriteMSR`, which both make use of this idea.
